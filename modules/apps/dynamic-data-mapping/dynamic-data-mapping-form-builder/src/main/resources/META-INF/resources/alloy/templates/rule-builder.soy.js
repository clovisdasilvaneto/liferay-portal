...
var $rule_list = function(opt_data, opt_ijData, opt_ijData_deprecated) {
  ...
  incrementalDom.elementOpenStart('div');
      incrementalDom.attr('class', 'ddm-rule-list');
  incrementalDom.elementOpenEnd();
  if ((rules.length) > 0) {
    incrementalDom.elementOpenStart('ul');
        incrementalDom.attr('class', 'ddm-form-body-content form-builder-rule-builder-rules-list tabular-list-group');
    incrementalDom.elementOpenEnd();
    var rule1185List = rules;
    var rule1185ListLen = rule1185List.length;
    for (var rule1185Index = 0; rule1185Index < rule1185ListLen; rule1185Index++) {
      var rule1185Data = rule1185List[rule1185Index];
      ...
      var condition1190List = rule1185Data.conditions;
      var condition1190ListLen = condition1190List.length;
      for (var condition1190Index = 0; condition1190Index < condition1190ListLen; condition1190Index++) {
        var condition1190Data = condition1190List[condition1190Index];
        $condition({operandType: condition1190Data.operands[0].type, operandValue: condition1190Data.operands[0].label, strings: strings}, opt_ijData);
        incrementalDom.elementOpenStart('b');
            incrementalDom.attr('class', 'text-lowercase');
        incrementalDom.elementOpenEnd();
        incrementalDom.elementOpen('em');
        incrementalDom.text(' ');
        soyIdom.print(strings[condition1190Data.operator]);
        incrementalDom.text(' ');
        incrementalDom.elementClose('em');
        incrementalDom.elementClose('b');
        if (condition1190Data.operands[1]) {
          $condition({operandType: condition1190Data.operands[1].type, operandValue: (condition1190Data.operands[1].label != null) ? condition1190Data.operands[1].label : condition1190Data.operands[1].value, strings: strings}, opt_ijData);
        }
        if (!(condition1190Index == condition1190ListLen - 1)) {
          incrementalDom.elementOpen('br');
          incrementalDom.elementClose('br');
          incrementalDom.elementOpen('b');
          incrementalDom.text(' ');
          soyIdom.print(strings[rule1185Data.logicOperator]);
          incrementalDom.text(' ');
          incrementalDom.elementClose('b');
        }
      }
      incrementalDom.elementOpen('br');
      incrementalDom.elementClose('br');
      var action1216List = rule1185Data.actions;
      ...
      for (var action1216Index = 0; action1216Index < action1216ListLen; action1216Index++) {
        ...
        $action({action: action1216Data}, opt_ijData);
      ...
      incrementalDom.elementOpenStart('li');
          ...
          incrementalDom.attr('data-card-id', rule1185Index);
      incrementalDom.elementOpenEnd();
...
/**
 * @param {$rule_types.Params} opt_data
 * @param {Object<string, *>=} opt_ijData
 * @param {Object<string, *>=} opt_ijData_deprecated
 * @return {void}
 * @suppress {checkTypes|uselessCode}
 */
var $rule_types = function(opt_data, opt_ijData, opt_ijData_deprecated) {
  opt_ijData = opt_ijData_deprecated || opt_ijData;
