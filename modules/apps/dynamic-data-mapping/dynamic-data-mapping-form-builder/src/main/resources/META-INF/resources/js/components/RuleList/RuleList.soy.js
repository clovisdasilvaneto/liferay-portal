...
goog.loadModule(function(exports) {
	...
	function $render(opt_data, opt_ijData, opt_ijData_deprecated) {
		...
		var dropdownExpandedIndex = soy.asserts.assertType(
			opt_data.dropdownExpandedIndex == null ||
				goog.isNumber(opt_data.dropdownExpandedIndex),
			'dropdownExpandedIndex',
			opt_data.dropdownExpandedIndex,
			'null|number|undefined'
		);
		/** @type {!Array<{actions: !Array<{action: (!goog.soy.data.SanitizedContent|string), dataProviderName: (?), expression: (?), inputLabel: (?), label: (!goog.soy.data.SanitizedContent|string), outputLabel: (?), target: (!goog.soy.data.SanitizedContent|string)}>, conditions: !Array<{operands: !Array<{label: (!goog.soy.data.SanitizedContent|string), type: (!goog.soy.data.SanitizedContent|string), value: (!goog.soy.data.SanitizedContent|string)}>, operator: (!goog.soy.data.SanitizedContent|string)}>, logicalOperator: (!goog.soy.data.SanitizedContent|string)}>} */
		var rules = soy.asserts.assertType(
			goog.isArray(opt_data.rules),
			'rules',
			opt_data.rules,
			'!Array<{actions: !Array<{action: (!goog.soy.data.SanitizedContent|string), dataProviderName: (?), expression: (?), inputLabel: (?), label: (!goog.soy.data.SanitizedContent|string), outputLabel: (?), target: (!goog.soy.data.SanitizedContent|string)}>, conditions: !Array<{operands: !Array<{label: (!goog.soy.data.SanitizedContent|string), type: (!goog.soy.data.SanitizedContent|string), value: (!goog.soy.data.SanitizedContent|string)}>, operator: (!goog.soy.data.SanitizedContent|string)}>, logicalOperator: (!goog.soy.data.SanitizedContent|string)}>'
		);
		...
		var strings = soy.asserts.assertType(
			goog.isObject(opt_data.strings),
			'strings',
			opt_data.strings,
			'!Object<!goog.soy.data.SanitizedContent|string,!goog.soy.data.SanitizedContent|string>'
		);
		/** @type {*} */
		...
		incrementalDom.elementOpenEnd();
		/** @desc  */
		var MSG_EXTERNAL_3368967445947154698 = Liferay.Language.get(
			'rule-builder'
		);
		incrementalDom.text(
			goog.string.unescapeEntities(MSG_EXTERNAL_3368967445947154698)
		);
		incrementalDom.elementClose('h1');
		incrementalDom.elementOpenStart('div');
		incrementalDom.attr(
			'class',
			'liferay-ddm-form-rule-rules-list-container'
		);
		incrementalDom.elementOpenEnd();
	...
	function $rule_list(opt_data, opt_ijData, opt_ijData_deprecated) {
		...
		incrementalDom.attr(
			'class',
			'ddm-rule-list-container form-builder-rule-list'
		);
		incrementalDom.elementOpenEnd();
		if (rules.length > 0) {
			...
			incrementalDom.attr(
				'class',
				'ddm-form-body-content form-builder-rule-builder-rules-list tabular-list-group'
			);
			incrementalDom.elementOpenEnd();
			...
			for (
				...
				incrementalDom.attr(
					'class',
					'clamp-horizontal list-group-item-content'
				);
				incrementalDom.elementOpenEnd();
				incrementalDom.elementOpenStart('p');
				incrementalDom.attr(
					'class',
					'form-builder-rule-builder-rule-description text-default'
				);
				incrementalDom.elementOpenEnd();
				incrementalDom.elementOpen('b');
				/** @desc  */
				var MSG_EXTERNAL_5099375131746050090 = Liferay.Language.get(
					'if'
				);
				incrementalDom.text(
					goog.string.unescapeEntities(
						MSG_EXTERNAL_5099375131746050090
					)
				);
				incrementalDom.text(' ');
				...
				for (
					var condition3713Index = 0;
					condition3713Index < condition3713ListLen;
					condition3713Index++
				) {
					var condition3713Data =
						condition3713List[condition3713Index];
					$condition(
						{
							operandType: condition3713Data.operands[0].type,
							operandValue: condition3713Data.operands[0].value
						},
						null,
						opt_ijData
					);
					...
					if (condition3713Data.operands[1]) {
						$condition(
							{
								operandLabel:
									condition3713Data.operands[1].label,
								operandType: condition3713Data.operands[1].type,
								operandValue:
									condition3713Data.operands[1].value
							},
							null,
							opt_ijData
						);
					...
					if (!(condition3713Index == condition3713ListLen - 1)) {
						...
						if (rule3757Data.logicalOperator == 'and') {
							/** @desc  */
							var MSG_EXTERNAL_3929281485914893910 = Liferay.Language.get(
								'and'
							);
							incrementalDom.text(
								goog.string.unescapeEntities(
									MSG_EXTERNAL_3929281485914893910
								)
							);
						} else {
							/** @desc  */
							var MSG_EXTERNAL_6627551976444260400 = Liferay.Language.get(
								'or'
							);
							incrementalDom.text(
								goog.string.unescapeEntities(
									MSG_EXTERNAL_6627551976444260400
								)
							);
						}
						incrementalDom.text(' ');
				...
				for (
					var action3734Index = 0;
					action3734Index < action3734ListLen;
					action3734Index++
				) {
					var action3734Data = action3734List[action3734Index];
					$action(
						{
							action: action3734Data.action,
							dataProviderName: action3734Data.dataProviderName,
							inputLabel: action3734Data.inputLabel,
							expression: action3734Data.expression,
							label: action3734Data.label,
							logicalOperator: rule3757Data.logicalOperator,
							outputLabel: action3734Data.outputLabel
						},
						null,
						opt_ijData
					);
					if (!(action3734Index == action3734ListLen - 1)) {
						...
						incrementalDom.text(' ');
						/** @desc  */
						var MSG_EXTERNAL_3929281485914893910$$1 = Liferay.Language.get(
							'and'
						);
						incrementalDom.text(
							goog.string.unescapeEntities(
								MSG_EXTERNAL_3929281485914893910$$1
							)
						);
						incrementalDom.text(' ');
						incrementalDom.elementClose('b');
				...
				var isExpanded__soy3739 =
					dropdownExpandedIndex == ruleIndex__soy3737;
				incrementalDom.elementOpenStart('div');
				...
				$templateAlias1(
					{
						items: rulesCardOptions,
						expanded: isExpanded__soy3739,
						events: {itemClicked: _handleRuleCardClicked},
						spritemap: spritemap
					},
					null,
					opt_ijData
			...
			/** @desc  */
			var MSG_EXTERNAL_669100315437640172 = Liferay.Language.get(
	...
	function $empty_list(opt_data, opt_ijData, opt_ijData_deprecated) {
		...
		var message = soy.asserts.assertType(
			opt_data.message == null ||
				(goog.isString(opt_data.message) ||
					opt_data.message instanceof goog.soy.data.SanitizedContent),
			'message',
			opt_data.message,
	...
	function $label(opt_data, opt_ijData, opt_ijData_deprecated) {
		...
		opt_data = opt_data || {};
		/** @type {!goog.soy.data.SanitizedContent|null|string|undefined} */
		var content = soy.asserts.assertType(
			opt_data.content == null ||
				(goog.isString(opt_data.content) ||
					opt_data.content instanceof goog.soy.data.SanitizedContent),
			'content',
			opt_data.content,
			'!goog.soy.data.SanitizedContent|null|string|undefined'
		);
	...
	function $condition(opt_data, opt_ijData, opt_ijData_deprecated) {
		opt_ijData = opt_ijData_deprecated || opt_ijData;
		/** @type {!goog.soy.data.SanitizedContent|string} */
		var operandType = soy.asserts.assertType(
			goog.isString(opt_data.operandType) ||
				opt_data.operandType instanceof goog.soy.data.SanitizedContent,
			'operandType',
			opt_data.operandType,
			'!goog.soy.data.SanitizedContent|string'
		);
		/** @type {!goog.soy.data.SanitizedContent|null|string|undefined} */
		var operandLabel = soy.asserts.assertType(
			opt_data.operandLabel == null ||
				(goog.isString(opt_data.operandLabel) ||
					opt_data.operandLabel instanceof
						goog.soy.data.SanitizedContent),
			'operandLabel',
			opt_data.operandLabel,
			'!goog.soy.data.SanitizedContent|null|string|undefined'
		);
		/** @type {?} */
		...
		if (
			...
			/** @desc  */
			var MSG_EXTERNAL_3375720161310377589 = Liferay.Language.get(
				'value'
			);
			incrementalDom.text(
				goog.string.unescapeEntities(MSG_EXTERNAL_3375720161310377589)
			);
			incrementalDom.text(' ');
			incrementalDom.elementClose('span');
			...
			incrementalDom.text(
				goog.string.unescapeEntities(MSG_EXTERNAL_123010868147850959)
			);
			incrementalDom.text(' ');
			incrementalDom.elementClose('span');
			...
			incrementalDom.elementOpen('span');
			/** @desc  */
			var MSG_EXTERNAL_8167904999979308163 = Liferay.Language.get(
				'field'
			);
			incrementalDom.text(
				goog.string.unescapeEntities(MSG_EXTERNAL_8167904999979308163)
			);
			incrementalDom.text(' ');
			incrementalDom.elementClose('span');
			...
			incrementalDom.text(
				goog.string.unescapeEntities(MSG_EXTERNAL_7246196759043272468)
			);
			incrementalDom.text(' ');
			incrementalDom.elementClose('span');
		}
	...
	function $action(opt_data, opt_ijData, opt_ijData_deprecated) {
		opt_ijData = opt_ijData_deprecated || opt_ijData;
		/** @type {!goog.soy.data.SanitizedContent|string} */
		var action = soy.asserts.assertType(
			goog.isString(opt_data.action) ||
				opt_data.action instanceof goog.soy.data.SanitizedContent,
			'action',
			opt_data.action,
			'!goog.soy.data.SanitizedContent|string'
		);
		...
		if (action == 'auto-fill') {
			...
			incrementalDom.elementOpen('b');
			/** @desc  */
			var MSG_EXTERNAL_8930607439260873448 = Liferay.Language.get(
				'autofill-x-from-data-provider-x'
			);
			MSG_EXTERNAL_8930607439260873448 = MSG_EXTERNAL_8930607439260873448.replace(
				/{(\d+)}/g,
				'\x01$1\x01'
			);

			var lastIndex_3859 = 0,
				partRe_3859 = /\x01\d+\x01/g,
				match_3859;
			do {
				match_3859 =
					partRe_3859.exec(MSG_EXTERNAL_8930607439260873448) ||
					undefined;
				incrementalDom.text(
					goog.string.unescapeEntities(
						MSG_EXTERNAL_8930607439260873448.substring(
							lastIndex_3859,
							match_3859 && match_3859.index
						)
					)
				);
			...
			incrementalDom.elementOpen('b');
			/** @desc  */
			var MSG_EXTERNAL_3206084286330742941 = Liferay.Language.get(
				'calculate-field-x-as-x'
			);
			MSG_EXTERNAL_3206084286330742941 = MSG_EXTERNAL_3206084286330742941.replace(
				/{(\d+)}/g,
				'\x01$1\x01'
			);

			var lastIndex_3882 = 0,
				partRe_3882 = /\x01\d+\x01/g,
				match_3882;
			do {
				match_3882 =
					partRe_3882.exec(MSG_EXTERNAL_3206084286330742941) ||
					undefined;
				incrementalDom.text(
					goog.string.unescapeEntities(
						MSG_EXTERNAL_3206084286330742941.substring(
							lastIndex_3882,
							match_3882 && match_3882.index
						)
					)
				);
			...
			incrementalDom.elementOpen('b');
			/** @desc  */
			var MSG_EXTERNAL_6578734339769388749 = Liferay.Language.get(
				'enable'
			);
			incrementalDom.text(
				goog.string.unescapeEntities(MSG_EXTERNAL_6578734339769388749)
			);
			incrementalDom.text(' ');
			incrementalDom.elementClose('b');
			incrementalDom.elementClose('span');
			...
			incrementalDom.elementOpen('b');
			/** @desc  */
			var MSG_EXTERNAL_6089475641791297161 = Liferay.Language.get(
				'jump-to-page'
			);
			incrementalDom.text(
				goog.string.unescapeEntities(MSG_EXTERNAL_6089475641791297161)
			);
			incrementalDom.text(' ');
			incrementalDom.elementClose('b');
			incrementalDom.elementClose('span');
			...
			incrementalDom.elementOpen('b');
			/** @desc  */
			var MSG_EXTERNAL_7337980473456458003 = Liferay.Language.get(
				'require'
			);
			incrementalDom.text(
				goog.string.unescapeEntities(MSG_EXTERNAL_7337980473456458003)
			);
			incrementalDom.text(' ');
			incrementalDom.elementClose('b');
			incrementalDom.elementClose('span');
			...
			incrementalDom.text(
				goog.string.unescapeEntities(MSG_EXTERNAL_1504349909398224884)
			);
			incrementalDom.text(' ');
			incrementalDom.elementClose('b');
			incrementalDom.elementClose('span');
	...
	exports.render.types = {
		dropdownExpandedIndex: 'number',
		rules:
			'list<[\n\t\t\tconditions: list<[\n\t\t\t\toperator: string,\n\t\t\t\toperands: list<[\n\t\t\t\t\ttype: string,\n\t\t\t\t\tlabel: string,\n\t\t\t\t\tvalue: string\n\t\t\t\t]>\n\t\t\t]>,\n\t\t\tactions: list<[\n\t\t\t\taction: string,\n\t\t\t\tdataProviderName: ?,\n\t\t\t\texpression: ?,\n\t\t\t\tinputLabel: ?,\n\t\t\t\ttarget: string,\n\t\t\t\tlabel: string,\n\t\t\t\toutputLabel: ?\n\t\t\t]>,\n\t\t\tlogicalOperator: string\n\t\t]>\n\t',
		rulesCardOptions: '?',
		spritemap: 'string',
		strings: 'map<string, string>',
		_handleDropdownClicked: 'any',
		_handleRuleCardClicked: 'any'
	};
	exports.rule_list.params = [
		'rules',
		'spritemap',
		'strings',
		'_handleDropdownClicked',
